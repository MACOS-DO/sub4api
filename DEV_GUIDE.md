...
| 项目 | 说明 |
|------|------|
| **项目仓库** | MACOS-DO/sub4api |
| **Fork 仓库** | bayma888/sub4api-bmai |
...
| 数据库凭据 | user=`sub4api`, password=`sub4api`, dbname=`sub4api` |
...
psql -U sub4api -h 127.0.0.1 -d sub4api -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER sub4api WITH PASSWORD 'sub4api';
...
```bash
# 连接数据库
psql -U sub4api -h 127.0.0.1 -d sub4api
...
# 执行 SQL 文件
psql -U sub4api -h 127.0.0.1 -d sub4api -f migration.sql
```
...
- [项目仓库](https://github.com/MACOS-DO/sub4api)
