  ...
  if (!key) return
	  ...
	  try {
        ...
        items: parsedItems.value,
	      ...
	      `sub4api-ui-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
	    )
...
async function retryFailedJob(job: BatchImageJobRow | BatchImageJob) {
  ...
  try {
    ...
    const retryJob = await submitBatchImageJob(
      ...
      `sub4api-ui-retry-${job.id}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
    )
