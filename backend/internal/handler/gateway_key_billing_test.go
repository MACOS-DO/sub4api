...
func newKeyBillingContext(apiKey *service.APIKey) (*gin.Context, *httptest.ResponseRecorder) {
	...
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/sub4api/billing", nil)
	if apiKey != nil {
...
func TestGatewayHandlerKeyBillingInfoUsesGroupRate(t *testing.T) {
	...
	require.Equal(t, "sub4api.key_billing", got.Object)
...
func TestGatewayHandlerLegacyKeyBillingObject(t *testing.T) {
	groupID := int64(7)
	apiKey := &service.APIKey{UserID: 11, GroupID: &groupID, Group: &service.Group{ID: groupID, RateMultiplier: 1}}
	c, w := newKeyBillingContext(apiKey)
	c.Request.URL.Path = "/v1/sub2api/billing"
	newKeyBillingHandler(nil).KeyBillingInfo(c)
	require.Equal(t, http.StatusOK, w.Code)
	var got keyBillingInfoResponse
	require.NoError(t, json.Unmarshal(w.Body.Bytes(), &got))
