...
func TestOpenAIImageOutputCounter_NestedSub4API_StandardResponse(t *testing.T) {
	// Simulate what a nested sub4api setup receives
	// When the upstream sub4api returns a standard response, the downstream
	// sub4api receives SSE events. This test verifies that no false positives
	// occur.
...
data: [DONE]`
	...
	count := countOpenAIImageOutputsFromSSEBody(sseBody)
	if count != 0 {
		t.Errorf("expected 0 images for nested sub4api text-only response, got %d", count)
	}
}
