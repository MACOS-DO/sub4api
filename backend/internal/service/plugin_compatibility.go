...
func EvaluatePluginCompatibility(manifest PluginManifest, host PluginHostInfo) PluginCompatibility {
	result := PluginCompatibility{
		CurrentSub4API:     host.Version,
		RequiredSub4API:    manifest.Requires.Sub4API,
		RecommendedSub4API: manifest.Requires.RecommendedSub4APIVersion,
	...
	if manifest.Requires.PluginProtocol != pluginv1.ProtocolVersion ||
		...
		result.Message = "插件协议版本与当前 Sub4API 不兼容"
		return result
	}
	if !matchesSemverRange(host.Version, manifest.Requires.Sub4API) {
		result.Status = "incompatible"
		result.Message = fmt.Sprintf("当前 Sub4API %s 不满足插件要求 %s", host.Version, manifest.Requires.Sub4API)
		return result
	}
	result.Compatible = true
	for _, tested := range manifest.Requires.TestedSub4APIVersions {
	...
	if result.Tested {
		...
		result.Message = "当前 Sub4API 版本已由插件声明测试"
	} else {
		result.Status = "untested"
		result.Message = "版本范围兼容，但插件未声明已测试当前 Sub4API 版本"
	}
	return result
}
