...
### 目标
本文档用于对接外部支付系统（如 `sub4apipay`）与 Sub4API 的 Admin API，覆盖：
- 支付成功后充值
- 用户查询
- 人工余额修正
- 前端购买页参数透传
...
```json
{
  ...
  "value": 100.0,
  "user_id": 123,
  "notes": "sub4apipay order: cm1234567890"
}
```
...
当 Sub4API 打开 `purchase_subscription_url` 或用户侧自定义页面 iframe URL 时，会统一追加：
- `user_id`
- `token`
- `theme`（`light` / `dark`）
...
### 6) `doc_url` 配置建议
- 查看链接：`https://github.com/MACOS-DO/sub4api/blob/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
- 下载链接：`https://raw.githubusercontent.com/MACOS-DO/sub4api/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
...
### Purpose
This document describes the minimal Sub4API Admin API surface for external payment integrations (for example, `sub4apipay`), including:
...
When Sub4API opens `purchase_subscription_url` or a user-facing custom page iframe URL, it appends:
- `user_id`
- `token`
- `theme` (`light` / `dark`)
...
- View URL: `https://github.com/MACOS-DO/sub4api/blob/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
- Download URL: `https://raw.githubusercontent.com/MACOS-DO/sub4api/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
